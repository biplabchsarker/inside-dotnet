...
## Chapters 009-010's five illustrations not yet sourced (2026-08-13, updated 2026-09-09)

Chapter 009 (Strings & Interning) and Chapter 010 (Garbage Collection Fundamentals) were each written in full — all 13 `article.md` sections, both `code/` projects per chapter built and run for real numbers, all 5 diagrams, and every companion file — but no image-generation/commissioning tool was available in the sessions that wrote them, so neither had its five signature illustrations, not even as an interim SVG placeholder (unlike Chapters 000-008, which all had *something* in these slots). Their content briefs are in [`ILLUSTRATION_BRIEFS.md`](ILLUSTRATION_BRIEFS.md#009--strings--interning) and [`ILLUSTRATION_BRIEFS.md`](ILLUSTRATION_BRIEFS.md#010--garbage-collection-fundamentals).

**2026-09-09: closed via interim SVG, not external sourcing.** The Canva/Figma connectors still aren't authorized (needs the project owner's action via claude.ai connector settings, outside any non-interactive session), and rather than leave two chapters fully bare, the project owner extended the Chapter 008 light-isometric hand-coded-SVG technique to 009 and 010 as well — see [IMAGE_GUIDE.md](IMAGE_GUIDE.md#interim-svg-placeholders-chapters-000-010)'s 2026-09-09 update. All five signature illustrations now exist for both chapters as SVG source (`diagrams/svg/`) plus rasterized PNG, following each chapter's brief above. This is still an interim placeholder, not the final externally-sourced version — the "Illustration quality ceiling" entry below still applies, and these SVGs should not be iterated on further; they get replaced wholesale once external sourcing is actually wired up.

**Status convention:** `book.json`'s `status` map used `"drafted (text/code/diagrams complete, images pending external sourcing)"` while a chapter had no illustrations at all. Now that 009 and 010 each have interim SVG placeholders for all five slots — the same condition Chapters 000-008 are considered `"done"` under — both moved to `"done"`, matching that existing precedent. `"drafted"` remains available for a future chapter that ships with genuinely zero images in any slot, same as 009/010 did before this update.
...
- **Card table, write barriers, ephemeral GC segments** — belong in [Episode 12 — GC Generations & LOH](chapters/011-gc-generations-loh/) (chapter not yet written)
