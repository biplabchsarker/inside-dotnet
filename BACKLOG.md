...
## Chapters 009-010's five illustrations not yet sourced (2026-08-13, updated 2026-08-14)

Chapter 009 (Strings & Interning) and Chapter 010 (Garbage Collection Fundamentals) were each written in full — all 13 `article.md` sections, both `code/` projects per chapter built and run for real numbers, all 5 diagrams, and every companion file — but no image-generation/commissioning tool was available in the sessions that wrote them, so neither has its five signature illustrations yet, not even as an interim SVG placeholder (unlike Chapters 000-008, which all have *something* in these slots). Their content briefs are ready in [`ILLUSTRATION_BRIEFS.md`](ILLUSTRATION_BRIEFS.md#009--strings--interning) and [`ILLUSTRATION_BRIEFS.md`](ILLUSTRATION_BRIEFS.md#010--garbage-collection-fundamentals) for whoever sources them.

**Status convention introduced by this gap:** `book.json`'s `status` map now uses `"drafted (text/code/diagrams complete, images pending external sourcing)"` for a chapter in this exact state, distinct from `"done"` (everything in [`WRITING_GUIDE.md`](WRITING_GUIDE.md)'s Definition of Done, images included) and `"planned"` (not started). Update a chapter's status to `"done"` once its five PNGs land in `images/`/`diagrams/png/` per its brief — don't leave it at `"drafted"` indefinitely, and don't reuse `"drafted"` for a chapter that's missing anything besides images. Expect this same gap to recur for each subsequent chapter until the image-sourcing pipeline (external commissioning, or an authorized Canva/Figma/AI image-gen connector) is actually wired up — see the "Illustration quality ceiling" entry below for the standing decision to source externally rather than resume hand-coded SVG.
